...
<div class="modal-container">
    <h1 class="content-subhead"><?php echo Yii::t('themes.spectre.main.main', 'Login to Your Account'); ?></h1>
    ...
    <?php if (!Yii::app()->user->isGuest): ?>
        <div class="alert alert-info">
            <?php echo Yii::t('themes.spectre.main.main', "{{headsup}} Looks like you're already logged in as {{email}}. You should {{logout}} before logging in to another account.", array(
                    '{{headsup}}' => CHtml::tag('strong', array(), Yii::t('themes.spectre.main.main', 'Heads Up!')),
                    '{{email}}'   => CHtml::tag('strong', array(), Yii::app()->user->email),
                    '{{logout}}'  => CHtml::tag('strong', array(), CHtml::link(Yii::t('themes.spectre.main.main', 'logout'), $this->createUrl('/logout')))
                )); ?>
         </div>
    <?php else: ?>
        ...
        <div class="pull-left">
            <?php echo CHtml::link(Yii::t('themes.spectre.main.main', 'register'), $this->createUrl('/register')); ?>
            <span> | </span>
    ...
    <?php if (Yii::app()->user->isGuest): ?>
        ...
        <div class="clearfix" style="border-bottom: 1px solid #aaa; margin: 15px;"></div>
            <span class="login-form-links"><?php echo Yii::t('themes.spectre.main.main', 'Or sign in with one of these social networks'); ?></span>
        <?php endif; ?>
        <div class="clearfix"></div>
