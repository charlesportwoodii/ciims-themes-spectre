var SpectreTheme = {
	...
	Blog : {
		...
		loadDisqus : function(shortname, id, title, slug) {
            ...
            SpectreTheme.Blog.loadDisqusCommentCount(shortname);
		},
		 ...
		 */
		commentBox : function() {
			SpectreTheme.endPoint = $("#endpoint").attr("data-attr-endpoint") + "/";
			$("#b").click( function () {
		        $(this).html("");
		        $("#a").slideDown("fast");
		        $("#submit-comment").show();
		    ...
		    $("#submit-comment").click(function(e) {
		        ...
		        $.post(SpectreTheme.endPoint + "/comment/comment", 
		        	{ 
		        		"Comments" : 
		        		{ 
		        			"comment" : $("#textbox").html(), 
		        			"content_id" : $(".content.no-padding").attr("data-attr-id") 
		        		}
		        	}, 
		        	function(data) { 
		...
		getComments : function(id) {
			...
			$.post(SpectreTheme.endPoint + "/comment/getComments/id/" + id, function(data) {
		...
		likeBox : function(id) {
			$("[id ^='upvote']").click(function(e) {
				...
				$.post(SpectreTheme.endPoint + "/content/like/id/" + id, function(data, textStatus, jqXHR) {
	...
	loadBlog : function(id) {
		...
		if (!$(".comments").hasClass("disqus"))	
			SpectreTheme.Blog.getComments(id);

		SpectreTheme.Blog.marked();
		SpectreTheme.Blog.commentBox();
		SpectreTheme.Blog.likeBox(id);
	},
